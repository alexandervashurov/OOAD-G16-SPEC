...
rel2intervals(Extension, X2Y, Y2X, XU, YU) :-
	...
	sort(XEv1, XEv4),
	sort(YEv1, YEv4),
	events2atoms(XEv4, XAtoms),
	events2atoms(YEv4, YAtoms),
...
split_extension(Extension, XAtoms, YAtoms, X2Y0) :-
	(   fromto(XAtoms,XAtoms1,XAtoms2,[]),
	    foreach(Xk-XAtoms1,KLX)
	do  XAtoms1 = [[Xk|_]|XAtoms2]
	),
	ord_list_to_avl(KLX, X2Tail),
	(   fromto(YAtoms,YAtoms1,YAtoms2,[]),
	    foreach(Yk-YAtoms1,KLY)
	do  YAtoms1 = [[Yk|_]|YAtoms2]
	),
	ord_list_to_avl(KLY, Y2Tail),
	(   foreach(Xs-Ys,Extension),
	    fromto(X2Y0,X2Y1,X2Y5,[]),
	    param(XAtoms,X2Tail,YAtoms,Y2Tail)
	do  (   foreach([Xi|Xj],Xs),
		...
		->  XAs2 = [(Xi..Xi)|XAs3]
		;   Xi = inf
		->  atoms_until(XAtoms, Xj, XAs2, XAs3)
		;   avl_fetch(Xi, X2Tail, Xtail),
		    atoms_until(Xtail, Xj, XAs2, XAs3)
		)
	    ),
	    (   foreach([Yi|Yj],Ys),
		...
		->  YAs2 = [(Yi..Yi)|YAs3]
		;   Yi = inf
		->  atoms_until(YAtoms, Yj, YAs2, YAs3)
		;   avl_fetch(Yi, Y2Tail, Ytail),
		    atoms_until(Ytail, Yj, YAs2, YAs3)
		)
	    ),
	    (   foreach(XA1,XAs1),
		...
		param(YAs1)
	    do  (   foreach(YA1,YAs1),
		    fromto(X2Y2,[XA1-YA1|X2Y3],X2Y3,X2Y4),
		    param(XA1)
		do  true
		)
	    )
...
atoms_until([[First|Stop]|_], Stop) --> !, [(First..Stop)].
atoms_until([[First|Later]|Tail], Stop) --> [(First..Later)],
	atoms_until(Tail, Stop).
