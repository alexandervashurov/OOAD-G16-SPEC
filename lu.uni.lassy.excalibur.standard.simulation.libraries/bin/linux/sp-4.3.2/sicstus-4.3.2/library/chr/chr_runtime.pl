...
/*  $Id: chr_runtime.pl,v 1.5 2013-05-08 12:40:42 perm Exp $
...
% Mats Carlsson 20080212
findall_chr_constraints(Cs) :-
	new(integer, Ctr),
        findall_chr_constraints(0, Ctr, Cs, []).
 
findall_chr_constraints(I, Ctr) -->
	{J is I+1},
	{put_contents(Ctr, contents, 0)},
	find_nth_constraint(J, Ctr), !,
	findall_chr_constraints(J, Ctr).
findall_chr_constraints(_, Ctr) -->
	{dispose(Ctr)}.

find_nth_constraint(J, Ctr) --> [C],
	{find_chr_constraint(C)},
	{get_contents(Ctr, contents, C1)},
	{C2 is C1+1},
	{put_contents(Ctr, contents, C2)},
	{C2=:=J}, !.
...
user:goal_expansion('chr get_mutable'(Val,Var),    (write(Position),nl,get_mutable(Val,Var))) :- prolog_load_context(term_position,Position), write(Position), nl.
...
% The formulation with chr_term_variables/2 is
...
'chr activate_constraint'( Vars, Susp, Generation) :-
	...
	( compound(State) ->			% passive/1
	    chr_term_variables( State, Vs),
	    'chr none_locked'( Vs),
	    Vars = [Global|Vs],
	    'chr default_store'(Global)
	; State == removed ->			% the price for eager removal ...
	    Susp =.. [_,_,_,_,_,_,_|Args],
	    chr_term_variables( Args, Vs),
	    Vars = [Global|Vs],
	    'chr default_store'(Global)
	;
	    Vars = []
...
leashed_ports(Set, Ports) :-
	...
	valid_ports(Ports, All, chr_leash(Set)).

valid_ports([], _, _).
valid_ports([H|T], Valid, Goal) :-
	(   memberchk(H, Valid)
	->  true
	;   % throw(error(domain_error(chr_port, H), _))
	    illarg(domain(term,chr_port), Goal, 1, H)
	),
	valid_ports(T, Valid, Goal).
...
chr_debug_interact(Event, Depth) :-
	leashed(Event), !,
	ask_query(chr_debugger, chr(event(Event, Depth)), trace_help, Command),
...
user:query_class_hook(chr_debugger, ' ? ', line,
	    char([creep-" cC\r\n", skip-"sS", ancestors-"gG", nodebug-"nN", abort-"aA", fail-"fF", break-"bB", help-"?hH", exit-[-1,0'e,0'E]]),
	    help_query) :- !.
...
handle_debug_command(Cmd, _, _) :-
	...
	illarg(domain(term,chr_debug_command), 0, 0, Cmd).

print_chr_debug_history :-
	get_debug_history(History,Depth),
	print_message(help, chr(ancestors(History, Depth))).

print_event(Event, Depth) :-
	print_message(help, chr(event(Event, Depth))).
...
set_debug_history(History,Depth) :-
	nb_setval(chr_debug_history,mutable(History,Depth)).

set_chr_debug(State) :-
	nb_setval(chr_debug,mutable(State)).
