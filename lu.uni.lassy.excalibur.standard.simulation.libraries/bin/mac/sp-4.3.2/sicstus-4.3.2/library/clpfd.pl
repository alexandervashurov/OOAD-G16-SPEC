...
suspension(ix(Ptr,FDPred,_,Ent,ZeroOne,_,_)) -->
	{var(Ent)}, !,
	propagator_goal(ix(Ptr,FDPred,_,Ent,ZeroOne,_,_)).
...
propagator_goal(ix(Ptr,FDPred,_,_,ZeroOne,_,_)) -->
...
fdpred_exported('oneof(x,y)=z IND'(X,Y,Z), X#=Y #\/ X#=Z).
...
fdpred_exported(bool_and(X,Y,Z), X #/\ Y #<=> Z).
fdpred_exported(bool_andn(X,Y,Z), X #/\ #\Y #<=> Z).
fdpred_exported(bool_nand(X,Y,Z), X #/\ Y #<=> #\Z).
fdpred_exported(bool_or(X,Y,Z), X #\/ Y #<=> Z).
fdpred_exported(bool_orn(X,Y,Z), X #\/ #\Y #<=> Z).
fdpred_exported(bool_nor(X,Y,Z), X #\/ Y #<=> #\Z).
fdpred_exported(bool_xor(X,Y,Z), X #\ Y #<=> Z).
fdpred_exported(bool_xorn(X,Y,Z), X #\ #\Y #<=> Z).
